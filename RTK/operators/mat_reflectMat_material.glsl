...
col.rgb = THIS_Color;
col.rgb+= texture(THIS_texture, pos.xz*vec2(THIS_Texscalex, THIS_Texscaley)).rgb*THIS_Texamount;
...
vec3 reflContrib = ref*THIS_Reflectionamount*((1-THIS_Fresnel)+(THIS_Fresnel*clamp(1-dot(-rd,n),0,1)));
col.rgb += reflContrib;
// col.rgb *= 2.6*exp(-0.2*t);
// col.rgb += ref*THIS_Reflectionamount*clamp(1-dot(-rd,n),0,1); //pseudo Fresnel

col.rgb += refraction*THIS_Transparency;
