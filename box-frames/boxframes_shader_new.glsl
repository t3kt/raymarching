...
#define Renderantialias 2
#define Reflectionpasses 1
#define Refractionpasses 2
#define Textureantialias 0

#define limitBox vec3(20.0, 10.0, 20.0)

#include "t_raymarch"
#include "rtk_generators"
#include "rtk_combiners"
...
Sdf sceneSDFInner_new(vec3 p)
{
    Sdf res = createSdf(10); // this needs to be fixed
    //float scene = sdPlane(p, uPlane);
    ...
    for (int i = 0; i < uNum-1; i++) {
        ...
        Sdf frameRes = gen_boxFrameSmooth(p, center, size, uFrameWidth * sizeThick.w, uFrameSmooth);
        res = opUnionSoft(res, frameRes, uSmoothK);
    }
    return res;
}
...
Sdf thismap(vec3 p) {
    Sdf block = gen_box(p, vec3(0, 0, -18), vec3(150, 150, 8));
...
//    pModMirrorXY(p, vec2(10, 10));
    ...
    Sdf scene = sceneSDFInner_new(p);

    scene = opUnionSoft(scene, block, uSmoothK*8);
    return scene;
}
